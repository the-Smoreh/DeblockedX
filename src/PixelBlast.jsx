...
void main() {
  ...
  if (uEnableRipples == 1) {
    for (int i = 0; i < MAX_CLICKS; ++i) {
      ...
      vec2 cuv = ((pos - uResolution * .5 - cellSize * .5) / uResolution) * vec2(aspectRatio, 1.0);
...
export default function PixelBlast({
  ...
  useEffect(() => {
    ...
    threeRef.current = { renderer, scene, material, quad, resizeObserver, raf, onPointerDown };
  ...
  return <div ref={containerRef} className={`pixel-blast-container ${className ?? ''}`.trim()} style={style} aria-hidden="true" />;
}
